...
| `COMM-06` | One channel, one post a day | Guard — `comm/one-slack-channel` in `scripts/lib/rule-checks.mjs` rejects any channel argument, any hardcoded `C0…`/`D0…` id and any of the retired channel env vars, everywhere except `src/lib/slack.ts`. It binds this repository only: the agent skills post to Slack too and live outside it, where four were found breaking this rule on 2026-08-20. See `OUTSTANDING.md`. |
