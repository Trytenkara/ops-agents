...
- Do not fabricate names or sign with a real person's name, and end with the team sign-off provided.
...
Also judge engagement: set "engaged" true when the supplier showed genuine interest or willingness IN OUR REQUESTED MATERIAL (gave pricing for it, asked a question about it, requested a sample/spec, said they can supply it, or otherwise moved that sourcing forward); false for a decline / "can't supply our material" / out-of-office / automated / off-topic message. A reply that declines our material and only pitches other materials we did not ask for is NOT engagement, so set "engaged" false and do not introduce held materials. But a supplier who CAN supply our material and only says they cannot ship it to the United States IS engaged, so set "engaged" true.

If "Other materials we also source from this supplier" is provided AND engaged is true, ALSO briefly introduce those materials in the same reply, naturally asking whether they can supply/quote them too (one short sentence or a compact list, never a wall of items). List the exact material names you introduced in "introduced_materials". If engaged is false, do NOT introduce them and return "introduced_materials": [].

Also judge whether this reply ENDS the conversation: set "conversation_complete" true only when your reply is a closing message that leaves the supplier nothing to answer, such as a thank-you / sign-off after a decline, or a wrap-up once everything we needed has been provided. Set it false whenever you asked for anything, acknowledged something we are still waiting on, or otherwise expect them to write back. When in doubt, false: this flag stops us from ever chasing them again.
    ...
    ...(atts.length
      ? [
          ...
          `The supplier ATTACHED ${atts.length} file(s) to this reply: ${atts
            ...
            .join(", ")}.`,
          `These files DID come through to us. Do NOT say the attachment is missing or ask them to resend it, and acknowledge receipt.`,
        ]
      : []),
