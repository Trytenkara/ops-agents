...
export const AGENT_SPECS: AgentSpec[] = [
  ...
  },
  {
    ...
    status: "shipped",
    cadence: "Every 5 min · America/New_York",
    purpose: "Catch supplier replies the webhook never delivered, chase suppliers who never answered the first email, and escalate the ones who still don't.",
    automatic:
      "Two jobs, in order. First it re-reads the Tenkara threads we are tracking, on a cadence based on how recently each was active, and replays any inbound message that never reached the webhook, so a reply cannot be lost because a single delivery failed. It takes the newest unseen message from each sender, and still files the attachments on older ones. Second, for threads with genuinely no reply, it stages two nudges (roughly 4 and 8 days out), pulling in any supplier contacts not yet reached, then opens a calling-escalation case. A nudge is held back when anyone on the thread has replied, when the previous nudge is still sitting unsent in the outbox, when the first email was never actually sent, or when the client is not Active. Everything is staged in the Tenkara thread; it never sends. Handling of replies that DID arrive lives on the Tenkara inbound webhook (Agent 08), not here: a scheduled agent can't react to a message, and this one exists to act on the message that never came.",
    humanInput:
      "Review the staged nudge in the Tenkara inbox and send it. Work the calling escalations. Track each thread to a captured price on the Pricing pipeline board. If a nudge did not go out, check whether the previous one is still unsent: that suppresses the next one on purpose.",
  },
  {
    number: 16,
