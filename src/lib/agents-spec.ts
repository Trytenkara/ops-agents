...
export const AGENT_SPECS: AgentSpec[] = [
  ...
  },
  {
    ...
    purpose: "Re-check current public pricing on Tenkara marketplace quotes that are expiring within 7 days.",
    automatic: "Pulls marketplace quotes expiring within 7 days, uses Anthropic web_search to find a current price signal per quote, and writes findings to marketplace_check_findings for ops review. Skips quotes with a pending finding. Read-only on Tenkara — never writes prices back.",
    humanInput: "Review the price findings and act on meaningful moves; the Live Price Index and material badges surface the deltas.",
  },
  {
    number: 6,
  ...
  },
  {
    ...
    automatic: "Scans for replies detected but not drafted, staged quotes missing a price or material, low-confidence or stale review items, and recent agent failures, then posts a summary of issues to Slack. Read-only — never mutates data.",
    humanInput: "Act on the flagged issues from the Slack summary (draft the missed reply, fix the incomplete quote, review the stale item).",
  },
  {
    number: 15,
