...
1. Overview: the scoreboard. Metric cards (new leads, drafts to send, quotes to approve, price changes, open cases, calls owed, pending approvals), each linking to where you act on it. "Open cases" and "Calls owed" split the same cases table: calls owed opens the Call Tracker, everything else opens the Cases page. The Sourcing card (admin/ops_lead) holds the org's sourcing status and its Tenkara inbox.
...
7. Call Tracker: the phone worklist. One collapsed row per call owed, filterable by operator and by call stage; open a row for the number, the supplier's calling window in their timezone, what we already emailed, where the inbox thread stands now (read live from Agent 13, so it is never a stale snapshot), and what to ask for. A row with no number on file has a box to add one, and the row links to the email thread. Outcomes are logged here, and a call is closed either by sending the supplier back into the email loop or by dropping them with a reason. Call rows are cases of type calling_escalation, so they do NOT show on the Overview "Open cases" card.
