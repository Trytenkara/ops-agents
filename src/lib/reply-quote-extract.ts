...
- price_source_text is MANDATORY whenever price is not null: copy the supplier's own words that state this price, exactly as written, including the number, its currency and its unit ("usd1280/mt FOB Qingdao", "Drums are 518lbs at 1.1975/lb"). One short fragment, not the whole email, and not your summary of it. It is checked by arithmetic: your price divided by your case_size has to equal a rate the fragment actually states, once the two units are converted, and a quote that does not reconcile against its own fragment is withheld from the client as unverifiable. Two consequences to be deliberate about. NEVER do arithmetic on a supplier's figure — not division by a pack size, and not multiplication by one either ("518lbs at 1.1975/lb" is a price of 1.1975, never 620.54). And report the unit THE SUPPLIER WROTE AGAINST THE NUMBER: "usd1280/mt" is per tonne, so unit_of_measurement is "MT" and never "kg", even when a kg figure appears elsewhere in the same sentence as a packing detail.
