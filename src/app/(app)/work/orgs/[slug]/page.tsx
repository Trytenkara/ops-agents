...
export default async function OrgOverview({ params }: { params: { slug: string } }) {
  ...
  const { data: org } = await admin
    .from("orgs")
    .select("id, slug, name, display_name, tenkara_org_id, is_internal, sourcing_status, onboarding_stage, bounce_alert_status, tenkara_email_account_id, tenkara_email_address")
    .eq("slug", params.slug)
    .maybeSingle();
  if (!org) notFound();
