...
export default async function ClientProfilePage({ params }: { params: { slug: string } }) {
  ...
  // settings. Surfaced read-only in the rep sheet so ops can confirm what
  // sourcing (Agent 03) and outreach (Agent 04) are suppressing. Best-effort —
  // a Tenkara read hiccup shouldn't break the profile.
  let dnc: DncValue | null = null;
  try {
    const detail = await getSourcingExclusionsDetail(org.tenkara_org_id);
    dnc = { ...detail, linked: !!org.tenkara_org_id };
  } catch {
    dnc = null;
  }
