...
function ensurePricingAsk(body: string, materials: string[]): string {
  ...
  const ask = `Could you share your EXW pricing (tiered/volume price breaks), lead time, and MOQ for ${list}? If available, please also include packaging details (case type, weight, and dimensions), payment terms, and any special handling requirements (e.g. refrigeration, hazmat classification). We can confirm exact volumes as we go.`;
