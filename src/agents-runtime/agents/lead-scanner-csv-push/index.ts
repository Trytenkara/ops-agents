...
registerAgent({
  ...
  async run(ctx) {
    ...
    for (const [key, group] of groups.entries()) {
      ...
      // 4d. Slack DM with CSV link.
      const text =
        `*New leads for ${group.supplier_name}* — ${group.rows.length} material${group.rows.length === 1 ? "" : "s"}, ` +
        `generated by Agent 11. Please upload to Lead Scanner.\n` +
        `CSV (signed, expires ${new Date(stored.expiresAt).toISOString().slice(0, 10)}): ${stored.signedUrl}`;
      const slackRes = await postSlackMessage({ channel: ANDREW_DM_ID, text });
      if (!slackRes.ok) {
        slackFailures++;
        await ctx.log(`Slack DM failed: ${slackRes.error}`, {
          level: "warn",
          step: "slack",
