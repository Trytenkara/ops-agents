...
export type ThreadKind = "outbound" | "inbound" | "expiring";
...
const KIND_META: Record<ThreadKind, { label: string; variant: string; title: string }> = {
  ...
  expiring: { label: "Expiring quote", variant: "warn", title: "Re-quote draft for a quote that's expiring or expired." },
};
...
const FILTERS: { value: "all" | ThreadKind; label: string }[] = [
  ...
  { value: "expiring", label: "Expiring quotes" },
];
...
export function ThreadsList({ rows, slug }: { rows: ThreadRow[]; slug: string }) {
  ...
  const counts = useMemo(() => {
    const c = { all: rows.length, outbound: 0, inbound: 0, expiring: 0 } as Record<string, number>;
